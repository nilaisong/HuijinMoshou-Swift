...
#pragma mark - 下拉刷新
- (MJRefreshMessageHeader *)addMessageHeaderWithRefreshingBlock:(void (^)())block
{
    return [self addMessageHeaderWithRefreshingBlock:block dateKey:nil];
}
- (MJRefreshMessageHeader *)addMessageHeaderWithRefreshingBlock:(void (^)())block dateKey:(NSString *)dateKey
{
    MJRefreshMessageHeader *header = [self addMessageHeader];
    header.refreshingBlock = block;
    header.dateKey = dateKey;
    return header;
}
- (MJRefreshMessageHeader *)addMessageHeaderWithRefreshingTarget:(id)target refreshingAction:(SEL)action
{
    return [self addMessageHeaderWithRefreshingTarget:target refreshingAction:action dateKey:nil];
}
- (MJRefreshMessageHeader *)addMessageHeaderWithRefreshingTarget:(id)target refreshingAction:(SEL)action dateKey:(NSString *)dateKey
{
    MJRefreshMessageHeader *header = [self addMessageHeader];
...
#pragma mark ----wangzz
//add by wangzz 161025
- (MJRefreshMessageHeader *)addMessageHeader
{
    MJRefreshMessageHeader *header = [[MJRefreshMessageHeader alloc] init];
    self.header = header;
...
- (MJRefreshMessageHeader *)messageHeader
{
    if ([self.header isKindOfClass:[MJRefreshMessageHeader class]]) {
        return (MJRefreshMessageHeader *)self.header;
    }
