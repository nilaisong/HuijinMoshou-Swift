...
@property (nonatomic, strong) NSMutableArray *deletedeArray;
...
-(id)initWithGroupURL:(NSURL *)assetsGroupURL andSelectedArr:(NSMutableArray *)arr{
    ...
    if (self) {
        ...
        _selectedAssetsArray = arr;
        _deletedeArray = [[NSMutableArray alloc]init];
...
////                self.selectedAssetsArray = [NSMutableArray arrayWithObject:[self dnImagePickerController].selectedArr];
//                for (ALAsset *asset in [self dnImagePickerController].selectedArr) {
//                    
//                    NSLog(@"zzzzzzz%@",[[[asset defaultRepresentation]url]debugDescription]);
//                }
//            }
    }
    return self;
...
//    }
    ...
    [self setupView];
    [self setupData];
    [self dnImagePickerController].isFirst = NO;
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    
    NSData *data = [user objectForKey:@"selecedImageArr"];
    
    self.currentSelectArray  = [NSKeyedUnarchiver unarchiveObjectWithData:data];
//    self.currentSelectArray =  [NSMutableArray arrayWithArray:[Tool getCache:@"selecedImageArr"]];
...
- (void)setupView
{
    ...
    item3 = [[UIBarButtonItem alloc] initWithCustomView:self.sendButton];
    ...
    if ([self dnImagePickerController].selectedArr.count>0) {
        ...
        self.navigationController.toolbarItems.firstObject.enabled = NO;
        item1.enabled = NO;
        ...
        [self.assetsGroup enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            if (result) {
                if ([self dnImagePickerController].selectedArr.count>0) {
                    ...
                    for (ALAsset *asset in [self dnImagePickerController].selectedArr) {
                        if ([[[[asset defaultRepresentation]url]debugDescription] isEqualToString:[[[result defaultRepresentation]url]debugDescription]]) {
                                                        [self assetIsSelected:result];

                        }
//                        NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
//                        NSURL *targetAssetURL = [result valueForProperty:ALAssetPropertyAssetURL];
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.currentSelectArray.count+self.selectedAssetsArray.count >= kDNImageFlowMaxSeletedNumber) {
...
//    [[self dnImagePickerController].selectedArr removeAllObjects];
    
    [self dnImagePickerController].selectedArr = [[NSMutableArray alloc]initWithArray:self.selectedAssetsArray];
    
                                                 //arrayWithObject:self.selectedAssetsArray] ;
//    for (ALAsset *asset in [self dnImagePickerController].selectedArr) {
//        NSLog(@"vvvvvv----%@",[[[asset defaultRepresentation]url]debugDescription]);
