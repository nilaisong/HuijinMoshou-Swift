...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=0; i<UUPICKER_MINUTE; i++) {
        ...
        if (0<i && i<=UUPICKER_MONTH)
            [monthArray appendObject:num];
        if (i<UUPICKER_HOUR)
            [hourArray appendObject:num];
        [minuteArray appendObject:num];
    }
    for (int i=UUPICKER_MINDATE; i<UUPICKER_MAXDATE; i++) {
        NSString *num = [NSString stringWithFormat:@"%d",i];
        [yearArray appendObject:num];
    }
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x, 108, 20, 20)];
    label.text = name;
...
{
    CGFloat startX = ([UIScreen mainScreen].bounds.size.width - 240) / 2.0 + 40;
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (isIOS7) {
            [self creatValuePointXs:@[[NSString stringWithFormat:@"%f",startX + 0],[NSString stringWithFormat:@"%f",startX + 55],[NSString stringWithFormat:@"%f",startX + 110],[NSString stringWithFormat:@"%f",startX + 165],[NSString stringWithFormat:@"%f",startX + 220]]
                          withNames:@[@"年",@"月",@"日",@"时",@"分"]];
        }
        return 5;
    }
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonth){
        if (isIOS7) {
            [self creatValuePointXs:@[@"175",@"275"]
                          withNames:@[@"年",@"月"]];
        }
        return 2;
    }
...
- (void)setdayArray:(NSInteger)num
{
    ...
    for (int i=1; i<=num; i++) {
        [dayArray appendObject:[NSString stringWithFormat:@"%02d",i]];
    }
}
