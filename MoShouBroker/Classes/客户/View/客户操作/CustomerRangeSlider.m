...
@implementation CustomerRangeSlider
...
- (UIImage *)trackBackgroundImage
{
    ...
    {
        UIImage* image = [self createImageWithColor:CustomerBorderColor];//[UIImage imageNamed:@"slider-default-trackBackground"];
...
- (UIImage *)trackImage
{
    if(_trackImage==nil)
    {
        UIImage* image = [self createImageWithColor:BLUEBTBCOLOR];//[UIImage imageNamed:@"slider-default-track"];
...
- (UIImage *)lowerHandleImageNormal
{
    ...
    {
        UIImage* image = [UIImage imageNamed:@"icon_price"];
...
- (CGRect)thumbRectForValue:(float)value image:(UIImage*) thumbImage
{
    ...
    float xValue = ((self.bounds.size.width-thumbRect.size.width)*((value - _minimumValue) / fabsf(_maximumValue - _minimumValue)));
...
- (void) addSubviews
{
    ...
    [self.trackBackground.layer setCornerRadius:2.0];
    [self.trackBackground.layer setMasksToBounds:YES];
    self.trackBackground.frame = [self trackBackgroundRect];
    
    //------------------------------
    // Track
    self.track = [[UIImageView alloc] initWithImage:self.trackImage];
    [self.track.layer setCornerRadius:2.0];
    [self.track.layer setMasksToBounds:YES];
...
-(BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (_upperValue-_lowerValue <= 1) {
        UIImageView *imgView = _lowerHandle;
        _lowerHandle = _upperHandle;
        _upperHandle = imgView;
        float value = _lowerValue;
        float touchOffset = _lowerTouchOffset;
        _lowerValue = _upperValue;
        _upperValue = value;
        _lowerTouchOffset = _upperTouchOffset;
        _upperTouchOffset = touchOffset;
...
{
    CGRect rect=CGRectMake(0.0f, 0.0f, 1.0f, 4.0f);
