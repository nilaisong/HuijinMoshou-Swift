...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [ChatUIHelper shareHelper].currentChatConversationId = self.conversation.conversationId;
...
- (void)setChatToolbar:(EaseChatToolbar *)chatToolbar
{
    ...
    tableFrame.size.height = self.view.frame.size.height- 64 - _chatToolbar.frame.size.height;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        timeCell.titleLabelColor = [UIColor whiteColor];
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:cellForMessageModel:)]) {
            ...
            if (cell) {
                if ([cell isKindOfClass:[EaseMessageCell class]]) {
                    ...
                    if ( model.isSender) {
                        emcell.messageTextColor = [UIColor whiteColor];
                    }else{
                        emcell.messageTextColor = [UIColor blackColor];
                    }
...
{
    int maxNum = [self convertToInt:text];
    if (maxNum>900) {
    
        AlertShow(@"您输入超过450个字符.");
