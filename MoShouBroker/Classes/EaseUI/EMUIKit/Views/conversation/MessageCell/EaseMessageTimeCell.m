...
- (void)_setupSubview
{
    timeBackgroundView = [[UIImageView alloc]init];
    //    UIImage* img=[UIImage imageNamed:@"时间背景"];//原图
    [timeBackgroundView setImage:[[UIImage imageNamed:@"时间背景"] stretchableImageWithLeftCapWidth:15 topCapHeight:10]];
    [self.contentView addSubview:timeBackgroundView];
    ...
    //    CGSize titleLabelSize = [_titleLabel boundingRectWithSize:CGSizeMake(1000, 17)];
    //    timeBackgroundView.frame = CGRectMake((kMainScreenWidth - titleLabelSize.width+20)/2, 7, titleLabelSize.width-20, 17);
...
- (void)setTitle:(NSString *)title
{
    ...
    NSLog(@"title:%lu",(unsigned long)title.length);
    UILabel *widthLabel = [[UILabel alloc]init];
    widthLabel.font = _titleLabel.font;
    widthLabel.text = title;
    CGSize widthLabelSize = [widthLabel boundingRectWithSize:CGSizeMake(1000, 17)];
    //    CGSize titleLabelSize = [_titleLabel boundingRectWithSize:CGSizeMake(1000, 17)];
    NSLog(@"%f",widthLabelSize.width);
    if (title.length > 10) {
        timeBackgroundView.frame = CGRectMake((kMainScreenWidth - 115)/2, 7, 115, 17);
    }
    else{
        timeBackgroundView.frame = CGRectMake((kMainScreenWidth - 75)/2, 7, 75, 17);
    }
}
