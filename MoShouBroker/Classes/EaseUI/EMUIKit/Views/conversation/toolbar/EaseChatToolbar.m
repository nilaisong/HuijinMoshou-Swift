...
//    _backgroundImageView.image = [[UIImage imageNamed:@"EaseUIResource.bundle/messageToolbarBg"] stretchableImageWithLeftCapWidth:0.5 topCapHeight:10];
   ...
   _ziXunTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 2, 30, 72/2)];
    _ziXunTitleLabel.text = @"我想";
    _ziXunTitleLabel.textColor = UIColorFromRGB(0x333333);
    _ziXunTitleLabel.font = FONT(12.f);
    [_toolbarView addSubview:_ziXunTitleLabel];
    
    _ziXunBuildingLabel = [[UILabel alloc]initWithFrame:CGRectMake(_ziXunTitleLabel.right+10, 7, 70, 22)];
    _ziXunBuildingLabel.text = @"咨询楼盘";
    _ziXunBuildingLabel.textColor = UIColorFromRGB(0x333333);
    _ziXunBuildingLabel.font = [UIFont boldSystemFontOfSize:12.f];
    _ziXunBuildingLabel.textAlignment = NSTextAlignmentCenter;
    _ziXunBuildingLabel.layer.cornerRadius = 10;
    _ziXunBuildingLabel.layer.masksToBounds = YES;
    _ziXunBuildingLabel.layer.borderColor =UIColorFromRGB(0x333333).CGColor;
    _ziXunBuildingLabel.layer.borderWidth = 0.5;
    _ziXunBuildingLabel.userInteractionEnabled = YES;
//    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(zixunTap)];
//    [_ziXunBuildingLabel addGestureRecognizer:tap];
    
    [_toolbarView addSubview:_ziXunBuildingLabel];
    
    //input textview
    _inputTextView = [[EaseTextView alloc] initWithFrame:CGRectMake(self.horizontalPadding,_ziXunTitleLabel.bottom, self.frame.size.width - self.verticalPadding * 2, self.frame.size.height - self.verticalPadding * 2-72/2)];
...
- (void)setInputViewRightItems:(NSArray *)inputViewRightItems
{
    ...
    CGFloat itemHeight = self.toolbarView.frame.size.height - self.verticalPadding * 2 -74/2;
        ...
        for (NSInteger i = (inputViewRightItems.count - 1); i >= 0; i--) {
            ...
            if ([item isKindOfClass:[EaseChatToolbarItem class]]) {
                ...
                if (chatItem.button) {
                    ...
                    itemFrame.origin.y = self.toolbarView.height - itemFrame.size.height-self.verticalPadding-5;           //(self.toolbarView.frame.size.height - itemFrame.size.height) / 2;
                    chatItem.button.frame = itemFrame;
...
- (void)_willShowInputTextViewToHeight:(CGFloat)toHeight
{
    ...
    DLog(@"  改变前  self.inputTextView.height==%f",self.inputTextView.height);
    
    
    DLog(@" 改变前  self.inputTextView.centerY==%f",self.inputTextView.centerY);
    ...
    if (toHeight == _previousTextViewContentHeight)
    {
        ...
        DLog(@"  改变后  self.inputTextView.height==%f",self.inputTextView.height);
        
        
        DLog(@" 改变后  self.inputTextView.centerY==%f",self.inputTextView.centerY);
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([text isEqualToString:@"\n"]) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            ...
            int maxNum = [self convertToInt:textView.text];
            if (maxNum>900) {
...
//            [self.delegate didSendText:chatText];
//            
//            int maxNum = [self convertToInt:chatText];
//            if (maxNum>900) {
//                
//            }else{
//                
...
//
//    
////    [[NSNotificationCenter defaultCenter] postNotificationName:@"CONSULTBUILDING" object:nil];
//
//    
//    
//}
