...
//
    ...
    for (UIView* view in self.navigationBar.subviews) {
        if ([view isMemberOfClass:[UILabel class]] || [view isMemberOfClass:[UIButton class]]) {
            [self.view insertSubview:view aboveSubview:maskView];
        }
    }
    
    [self.view insertSubview:_addScheduleBtn aboveSubview:self.navigationBar.titleLabel];
    [self.view insertSubview:_selectMonthButton aboveSubview:self.navigationBar.titleLabel];
    [self.view insertSubview:_todayBtn aboveSubview:self.navigationBar.titleLabel];
    [self hasNetwork];
...
- (void)calendar:(JTCalendarManager *)calendar prepareDayView:(JTCalendarDayView *)dayView
{
    ...
    if([_calendarManager.dateHelper date:[NSDate date] isTheSameDayThan:dayView.date]){
        ...
        if ([_calendarManager.dateHelper date:_selectedDate isTheSameDayThan:[NSDate date]]) {
            ...
            dayView.circleView.backgroundColor = [UIColor colorWithRed:0.11f green:0.62f blue:0.92f alpha:1.00f];
...
//    [self.menuLabel setText:[self.formatter stringFromDate:[_calendarManager date]]];
    [self.selectMonthButton setTitle:[self.formatter stringFromDate:[_calendarManager date]] forState:UIControlStateNormal];
    [self reloadInfo];
}
        ...
        [[DataFactory sharedDataFactory] getAllScheduleListWithCallBack:^(XTScheduleListResultModel *result) {
            ...
            for (int i = 0; i < result.remindList.count; i++) {
                ...
                if ([result isKindOfClass:[RemindResult class]]) {
                    [arrayM appendObject:[weakSelf.inputFormatter dateFromString:result.datetime]];
                }
            }
...
- (UIButton *)selectMonthButton{
    if (!_selectMonthButton) {
        ...
        CGFloat left = 4*9;
        monthBtn.frame = CGRectMake((kMainScreenWidth-120)/2, 20, 120, 44);
        ...
        monthBtn.titleLabel.font = [UIFont systemFontOfSize:18];
        [monthBtn setTitleColor:[UIColor colorWithRed:0.20f green:0.20f blue:0.20f alpha:1.00f] forState:UIControlStateNormal];
        [self.navigationBar addSubview:monthBtn];
...
//
        view.customerCallBack = ^(XTUserScheduleInfoCellEvent event,RemindResult* customer){
            switch (event) {
                ...
                {
                    MobileVisible* mobile = [customer.phoneList firstObject];
                    if (mobile) {
                        NSMutableString * str=[[NSMutableString alloc] initWithFormat:@"tel:%@",customer.phone];
                        if ([mobile.hidingPhone rangeOfString:@"*"].location == NSNotFound && mobile.hidingPhone.length > 0) {
                            str=[[NSMutableString alloc] initWithFormat:@"tel:%@",mobile.hidingPhone];
                        }else if(mobile.totalPhone.length > 0){
                            str=[[NSMutableString alloc] initWithFormat:@"tel:%@",mobile.totalPhone];
                        }
...
- (void)reloadInfo{
    ...
    if ([NetworkSingleton sharedNetWork].isNetworkConnection) {
        ...
        } failerCallBack:^(ActionResult *result) {
            [weakSelf showTips:result.message];
        }];
    }
