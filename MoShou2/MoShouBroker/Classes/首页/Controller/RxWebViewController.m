...
@interface RxWebViewController ()<UIWebViewDelegate,UINavigationBarDelegate,UIGestureRecognizerDelegate>

//@property (strong, nonatomic) UIBarButtonItem* customBackBarItem;
@property (strong, nonatomic) UIButton* closeButtonItem;
...
-(void)startPopSnapshotView{
    ...
    CGPoint center = CGPointMake(self.view.bounds.size.width/2,(self.view.bounds.size.height - 64)/2);
...
//        [self.navigationItem setLeftBarButtonItems:@[self.closeButtonItem] animated:NO];
        self.closeButtonItem.hidden = NO;
...
//        self.navigationController.interactivePopGestureRecognizer.enabled = YES;
        self.popGestureRecognizerEnable = YES;
//        self.closeButtonItem.hidden = YES;
...
/**
 *  由有无上一页决定webview是否处理拖动事件
 */
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    return _webView.canGoBack && !_isSwipingBack;
}
...
- (void)updateHostLabelWithRequest:(NSURLRequest *)request {
    ...
    if (host) {
        self.hostInfoLabel.text = [NSString stringWithFormat:@"网页由 汇金行 提供"];
    }
}
...
- (BOOL)checkRequest:(NSURLRequest*)request{
    ...
    if ([components count] > 1 && [(NSString *)[components objectAtIndex:0] isEqualToString:@"testapp"]) {
        if([(NSString *)[components objectAtIndex:1] isEqualToString:@"build"])
        {
//            self.navigationController.interactivePopGestureRecognizer.enabled = YES;
            self.popGestureRecognizerEnable = YES;
            BuildingDetailViewController* bVC = [[BuildingDetailViewController alloc]init];
            bVC.buildingId = [components objectAtIndex:2];
...
-(UIWebView*)webView{
    if (!_webView) {
        _webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 64, kMainScreenWidth, self.view.frame.size.height - 64)];
...
-(UIButton*)closeButtonItem{
    if (!_closeButtonItem) {
//        _closeButtonItem = [[UIButton alloc] initWithTitle:@"关闭" style:UIBarButtonItemStylePlain target:self action:@selector(closeItemClicked)];
        _closeButtonItem = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.navigationBar.leftBarButton.frame), self.navigationBar.leftBarButton.frame.origin.y, 44, self.navigationBar.leftBarButton.frame.size.height)];
        [_closeButtonItem setTitleColor:BLUEBTBCOLOR forState:UIControlStateNormal];
        [_closeButtonItem setTitle:@"关闭" forState:UIControlStateNormal];
        [_closeButtonItem addTarget:self action:@selector(closeItemClicked) forControlEvents:UIControlEventTouchUpInside];
        [self.navigationBar addSubview:_closeButtonItem];
        _closeButtonItem.hidden = YES;
    }
...
- (UIProgressView *)progressView {
    if (!_progressView) {
        CGRect frame = CGRectMake(0, 62, self.view.frame.size.width, 2.0);
