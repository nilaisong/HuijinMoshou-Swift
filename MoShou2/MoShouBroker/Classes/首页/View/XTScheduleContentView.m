...
//            [arrayM removeObjectForIndex:indexPath.row];
//        }
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0|| indexPath.row % 2 ==0 || _resultModel.remindList.count <= 0) {
        return NO;
    }else return YES;
}
...
- (void)setResultModel:(XTScheduleListResultModel *)resultModel{
    ...
    _noResult = resultModel.remindList.count <= 0;
    NSMutableArray* arrayM = [NSMutableArray array];
//    NSArray* remindListArray = [[resultModel.remindList reverseObjectEnumerator]allObjects];
    for(RemindResult* result in resultModel.remindList) {
        if (![result isKindOfClass:[RemindResult class]]) return;
        NSDate* date = [_inputFormatter dateFromString:result.datetime];
        result.datetime = [_hourFormatter stringFromDate:date];
        XTUserScheduleCellFrame* cellFrame = [[XTUserScheduleCellFrame alloc]initWithRemindResult:result];
        
        [arrayM addObject:cellFrame];
        [arrayM addObject:cellFrame];
    }
    
    _cellFrameArray = arrayM;
...
- (UIView *)noDataTipsView{
    if (!_noDataTipsView) {
        ...
        label.frame = CGRectMake(x, y,[label.text sizeWithfont:[UIFont systemFontOfSize:14] maxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)].width, 14);
        requestMoneyBtn.frame = CGRectMake(CGRectGetMaxX(label.frame), label.frame.origin.y, [requestMoneyBtn.titleLabel.text sizeWithfont:[UIFont systemFontOfSize:14] maxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)].width, 14);
