...
- (void)_setupSubview
{
    ...
    _bageNumberLabel = [[UILabel alloc]init];
//                        WithFrame:CGRectMake(kMainScreenWidth-30, self.height/2+20, 20, 20)];
    
    _bageNumberLabel.layer.cornerRadius = 10;
    
    _bageNumberLabel.layer.masksToBounds = YES;
    
    _bageNumberLabel.font = FONT(14.f);
    _bageNumberLabel.textColor = [UIColor whiteColor];
    _bageNumberLabel.textAlignment = NSTextAlignmentCenter;
    
    _bageNumberLabel.backgroundColor =ORIGCOLOR;
    [self.contentView addSubview:_bageNumberLabel];
    
    _ruleNamelabel = [[UILabel alloc]init];
    _ruleNamelabel.layer.cornerRadius = 3;
    _ruleNamelabel.layer.masksToBounds = YES;
    _ruleNamelabel.font = FONT(15.f);
    _ruleNamelabel.textColor = [UIColor whiteColor];
    _ruleNamelabel.textAlignment = NSTextAlignmentCenter;
    [self.contentView addSubview:_ruleNamelabel];
...
- (void)_setupTitleLabelConstraints
{
    ...
    self.titleWithAvatarLeftConstraint = [NSLayoutConstraint constraintWithItem:self.titleLabel attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.avatarView attribute:NSLayoutAttributeRight multiplier:1.0 constant:10+50+8]; //10+50+8   EaseConversationCellPadding
...
//    }
//    else{
//        self.titleLabel.text =[UserCacheManager getNickById:_model.conversation.conversationId];
//        // _model.conversation.conversationId;
//    }
    self.titleLabel.text =[UserCacheManager getNickById:_model.conversation.conversationId];
    ...
    if (_model.conversation.unreadMessagesCount == 0) {
        ...
        _bageNumberLabel.hidden = YES;
    }
    else{
        
        _bageNumberLabel.hidden = NO;

        NSDictionary *attributes = @{NSFontAttributeName:FONT(14)};
        NSString *unreadMessagesCount = [NSString stringWithFormat:@"%zd",_model.conversation.unreadMessagesCount];
        
        CGSize size = [unreadMessagesCount sizeWithAttributes:attributes];
        _bageNumberLabel.frame = CGRectMake(kMainScreenWidth-30, self.height/2, MAX(size.width+10, size.height),size.height);
        
        [_bageNumberLabel.layer setCornerRadius:size.height/2];
        _bageNumberLabel.text = unreadMessagesCount;

    }
    
    _ruleNamelabel.frame = CGRectMake(self.avatarView.right+10, self.timeLabel.top+5, 50, 18);
    if ([_model.conversation.conversationId rangeOfString:@"confirm"].location!=NSNotFound) {
        //确客
        _ruleNamelabel.backgroundColor = BLUEBTBCOLOR;
        _ruleNamelabel.text = @"确客";
    }else{
        _ruleNamelabel.backgroundColor =kRGB(42, 194, 144);
        _ruleNamelabel.text = @"客户";
    }
